...
logger = logging.getLogger("CineAgent.ClickHouse")
    ...
    def _init_connection(self):
        if self.host:
            try:
                ...
                logger.info(f"⚡ Connecting to LIVE ClickHouse Cloud Service at {self.host}:{self.port}...")
                ...
                logger.info(f"✅ LIVE ClickHouse Cloud Connected Successfully! (Ping: {ping_res})")
                self._setup_schema()
                return
            except Exception as e:
                logger.error(f"❌ Could not connect to Live ClickHouse Cloud host ({e}). Falling back to Embedded Engine.")
                self.client = None
        
        self.use_mock = True
        logger.info("Using Embedded ClickHouse Engine.")
        self._load_seed_mock_data()

    def _setup_schema(self):
        """Initializes tables in ClickHouse for scenes, dialogues, and script vector indices."""
        if self.use_mock or not self.client:
            logger.info("Using Embedded ClickHouse Engine; skipping table schema setup.")
            return
        ...
        try:
            ...
            """)
            logger.info("✅ ClickHouse schema tables (`scenes`, `dialogues`) created/verified.")
        except Exception as e:
    ...
    def insert_scene(self, scene_id: str, title: str, heading: str, description: str, tension: float, pacing: str, vector: List[float]):
        ...
        if not self.use_mock and self.client:
            try:
                ...
                logger.info(f"✅ Scene {scene_id} inserted into LIVE ClickHouse DB.")
                return
            except Exception as e:
    ...
    def vector_search_scenes(self, query_vector: List[float], limit: int = 3) -> List[Dict[str, Any]]:
        ...
        if not self.use_mock and self.client:
            try:
                ...
                logger.error(f"ClickHouse vector search query error: {e}")
    ...
    def get_telemetry_analytics(self) -> Dict[str, Any]:
        ...
        if not self.use_mock and self.client:
            try:
                ...
                logger.error(f"ClickHouse analytics query error: {e}")
